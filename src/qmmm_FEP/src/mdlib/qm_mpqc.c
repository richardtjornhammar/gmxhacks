...
//      if((qm->guessWF+zf)>1){
      ...
      fprintf(clustF[i]," name = \"%s\"\n )\n )",qm->guessWF==eGWFb?"6-31G":"STO-3G");//eQMbasis_names[qm->QMbasis]);
/*      } else {
        fprintf(clustF[i], "\n     guess_wavefunction<CLHF>: (\n       total_charge = %d\n       molecule = $:molecule\n       basis<GaussianBasisSet>: (\n          molecule = $:molecule\n          name = \"6-31G\"\n       )\n     )",anint002(tchrg[i]));
      }
*/
...
real read_mpqc(rvec QMgrad[],rvec MMgrad[], t_QMrec *qm, t_MMrec *mm, int I)
{
  ...
  if(!FAIL){
    for(i=0;i<nrQM;i++){
      ...
      if(qm->useQMatom[j]){ //this should prob. cover the whole lot
...
{
  /* normal mpqc jobs */
  int i,I,j,RV=0,cl=1,NFILES=1,NFILER=1,GWF,k,l,a,aa,b,bb;
  ...
  for(k=0;k<qm->nrQMatoms;k++){         // ADD Q ATOMS                   
    ...
    for(l=0;l<qm->nrQMatoms;l++){       // DISTANCE BETWEEN POINT AND THE CLUSTER THIS QM ATOM BELONGS TO 
      ...
      if((qm->QMcA[b*(qm->nrQMatoms)+bb]<=(qm->fQMelrc)) && (a!=aa) && (b!=bb) ) {	// IS THIS A QM ATOM THAT BELONG TO A DIFFERENT CLUSTER ?
        rvec_sub(qm->xQM[b],qm->xQM[bb],dist[0]);
        qmmmPBC(qm,dist);
	...
	r2 =rij*rij;
        ...
        for(j=XX;j<=ZZ;j++){
          ...
          f[b][j]     +=theFORCE;
          fshift[b][j]+=theFORCE;	//FOR SR VIRIAL 
        }
        DIener += C12ij-C6ij;
      }
    }
