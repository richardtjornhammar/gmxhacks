            ...
            bool              bQMMM)
{
    ...
    bFreeEnergy = FALSE; bQMenergy = FALSE;
    if (md->nPerturbed) 
    {
...
//NEW END    
    ...
    {
        vdwc = bFreeEnergy?&nlist[eNL_VDWQQ]:NULL;
        vdw =  bFreeEnergy?&nlist[eNL_VDW]:NULL;
...
*/
    ...
    bFreeEnergy &= !(fr->bQMblockFEP);
    // QMMM_GRP BLOCKS FEP_GRP ... MAYBE CHANGE THIS BACK ...
    // SO WE HAVE QM AND FEP ATOMS AND A BLOCKED CASE FOR THE QMMM BACKGROUND
    // IF QM AND NOT FEP OR FEP AND NOT QM THEN NORMAL
    // IF QM AND FEP AND BLOCK THEN BLOCK
    // IF QM AND FEP AND NOT BLOCK THEN FEP
    // IF [FEP and not block] AND [NOT QM AND bQMMM]
                       ...
                       bool bMakeQMMMnblist)
{
    ...
    for(icg=cg0; (icg < cg1); icg++)
    {
        ...
        if (bMakeQMMMnblist)//&& !(fr->bQMlambda)) //ALWAYS MAKE THE QMMM LIST ...?
        { 
                      ...
                      bool bFillGrid,bool bDoForces)
{
    ...
    //fprintf(stdout," Set some local variables \n");
    ...
    if (bGrid && bFillGrid)
    {
        ...
        debug_gmx();
        //fprintf(stdout,"Don't know why this all is... (DvdS 3/99) \n");
    ...
    if (bGrid)
    {
         ...
         */
        if (fr->bQMMM && fr->qr->QMMMscheme!=eQMMMschemeoniom ) //&& !(fr->bQMlambda))
        {
            fr->bQMblockFEP=TRUE;
            nsearch += nsgrid_core(log,cr,fr,box,box_size,ngid,top,
                                   grid,x,ns->bexcl,ns->bExcludeAlleg,
