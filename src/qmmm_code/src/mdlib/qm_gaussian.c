...
void init_gaussian(t_commrec *cr, t_QMrec *qm, t_MMrec *mm)
{
  ...
  system("ls -dalh clusters > filedump.qwerty");
  fci=fopen("filedump.qwerty","r");
  if(fgets(line,100,fci)==NULL){
    system("mkdir clusters");
    if(qm->bCLUSTER){ 
      for(i=0;i<qm->nrQMatoms;i++){
        sprintf(line,"mkdir clusters/c%d",i);
        system(line);
      }
    }else{
        sprintf(line,"mkdir clusters/c0");
        system(line);
    }
  }
  system("rm filedump.qwerty"); 
   ...
   */
    snew(buf,MAXEL);
    buf = getenv("GAUSS_NTHREADPERPROC");
    if (buf)
      sscanf(buf,"%d",&qm->nQMcpus);
    else
      qm->nQMcpus=1;
    fprintf(stderr,"number of threads per proc for gaussian = %d\n",qm->nQMcpus);

    snew(buf,MAXEL);
    buf = getenv("GAUSS_NPROCPERNODE");
    if (buf){
    ...
    fprintf(stderr,"number of procs per node for gaussian = %d\n",qm->nQMproc);

    snew(buf,MAXEL);
    buf = getenv("GAUSS_NNODEPERJOB");
    if (buf){
    ...
    fprintf(stderr,"number of nodes per job for gaussian = %d\n",qm->nQMnodes);
    ...
    snew(buf,MAXEL);  
    buf = getenv("GAUSS_MPI");
    if (buf){
      snew(qm->mpihandler,MAXEL);
      sscanf(buf,"%s",qm->mpihandler);
    }
    else
      gmx_fatal(FARGS,"no $GAUSS_MPI, check gaussian manual\n");
    fprintf(stderr,"\nGot mpi handle: %s\n",qm->mpihandler);   
    snew(buf,MAXEL);
...
void write_gaussian_input(int step ,t_forcerec *fr, t_QMrec *qm, t_MMrec *mm)
{
  ...
  for(cl=0;cl<NRC;cl++){
    sprintf(dstring,"clusters/c%d/input%d",cl,cl);
    clustF[cl]=fopen(strcat(dstring,estring),"w");
    if(step && qm->guessWF>=eGWFyes){
    ...
    if ((step && qm->guessWF>=eGWFyes)|| qm->QMmethod==eQMmethodCASSCF){
  ...
  for (i=0;i<qm->nrQMatoms;i++){
    ...
    fprintf(clustF[j],"%3d %10.7lf  %10.7lf  %10.7lf\n",
	    qm->atomicnumberQM[k],
	    qm->rQM[j*nrQM+k][XX]/BORH2NM,
	    qm->rQM[j*nrQM+k][YY]/BORH2NM,
	    qm->rQM[j*nrQM+k][ZZ]/BORH2NM);
#else
  ...
  for(cl=0;cl<NRC;cl++){
	...
	for(i=0;i<mm->nrMMatoms;i++){
	  ...
	  if(mm->MMcluster[ nrCOL*cl + i ]){
	    fprintf(clustF[cl],"%10.7lf  %10.7lf  %10.7lf %8.4lf\n",
		    mm->rMM[ nrCOL*cl + i ][XX]/BORH2NM,
		    mm->rMM[ nrCOL*cl + i ][YY]/BORH2NM,
		    mm->rMM[ nrCOL*cl + i ][ZZ]/BORH2NM,
		    mm->MMcharges[i]*mm->scalefactor);
	  }
#else
	  if(mm->MMcluster[ nrCOL*cl + i ]){
	...
	for(i=0;i<nrQM;i++){
	  ...
	  if(mm->MMcluster[i+nrMM+nrCOL*cl]){
	    fprintf(clustF[cl],"%10.7f  %10.7f  %10.7f %8.4f\n",
		    mm->rMM[ i+nrMM+nrCOL*cl ][XX]/BORH2NM,
		    mm->rMM[ i+nrMM+nrCOL*cl ][YY]/BORH2NM,
		    mm->rMM[ i+nrMM+nrCOL*cl ][ZZ]/BORH2NM,
		    qm->pcharges[i]*mm->scalefactor);
	  }
	}
      }
    }
...
real read_gaussian_output(rvec QMgrad[],rvec MMgrad[],int step,t_QMrec *qm, t_MMrec *mm, int I)
{
  ...
  char line[MAXEL],hang[2*MAXEL],lp1[100],lp2[100],mulliken[MAXEL],gname[100];
  ...
  sprintf(gname,"clusters/c%d/fort.7",I);
  in=fopen(gname,"r");
  /* 
...
//NDX FILES FOR VISUALISATION OF THE QMMM REGIME
  sprintf(nameMM,"clusters/c%d/mmatoms%d.ndx",I,I);
  sprintf(nameQA,"clusters/c%d/qaatoms%d.ndx",I,I);
  ...
  if(qm->QMmethod>=eQMmethodRHF && qm->includeQ>eIncludeNONE){  
    ...
    for(i=0;i<nrQM;i++){
      ...
      if(mm->MMcluster[(i+nrMM)+I*nrCOL]){
        fprintf(fqandx,"%10d",qm->indexQM[i]);
        if(++l==20){
          fprintf(fqandx,"\n");
          l=0;
        }
  ...
  if(qm->bPrint_npa){
    sprintf(g03fnm,"clusters/c%d/output%d.out",I,I);
    ...
    while(!feof(g03log) && flag){
      ...
      //add mut calc. from gaussian orbitals
      if( strstr(line,"Tot=") && strstr(line,"X=") ){
          ...
          if(line[5]=='1' && flag){                  // found entry    ( we have atoms belong to this cluster following )
            ...
            for(i=0;i<qm->nrQMatoms;i++){
              ...
              if(a==I){
                ...
                sprintf(hang,"%d %f %f %f %d %s",I,qm->rQM[b][XX]/BORH2NM,qm->rQM[b][YY]/BORH2NM,qm->rQM[b][ZZ]/BORH2NM,qm->indexQM[b],line);
...
void do_gaussian(int step, char *exe, int I,t_QMrec *qm)
{
  ...
  // makes one call to a small mpi execution environment for gaussian

  sprintf(inname,"input%d.com",I);
  sprintf(outname,"output%d.out",I);
  sprintf(in2name,"input%d_2.com",I);
  sprintf(ut2namn,"output%d_2.out",I);  
  qm->dI=(I+1);
...
void do_gaussian_spec(int step, char *exe, t_QMrec *qm)
{
  // just to run the command
  char runline[2000];
  // should really add custom line to send into mpirun
  sprintf(runline,"mpirun -npernode %d -np %d %s -v %d %s \"<\" input com \">\" output out ",qm->nQMproc,qm->nQMnodes,qm->mpihandler,qm->nrQMc,exe);
		   ...
		   t_QMrec *qm, t_MMrec *mm, rvec f[], rvec fshift[])
{
  ...
  /* Allocate */
  snew(QMgrad,qm->nrQMatoms*2);
  snew(MMgrad,mm->nrMMatoms); 
 
  for(i=0;i<qm->nrQMatoms;i++){
    QMgrad[i][XX]=0.0; QMgrad[i][YY]=0.0; QMgrad[i][ZZ]=0.0;
    QMgrad[i+qm->nrQMatoms][XX]=0.0; QMgrad[i+qm->nrQMatoms][YY]=0.0; QMgrad[i+qm->nrQMatoms][ZZ]=0.0;
  }
  for(i=0;i<mm->nrMMatoms;i++){
  ...
  aa=(2*(qm->nrQMatoms)+mm->nrMMatoms);
  bb=qm->nrQMatoms+mm->nrMMatoms;

  do_gaussian_spec(step,exe,qm);
  ...
  for(I=0;I<NFILES;I++){
    ...
    for(i=0;i<qm->nrQMatoms;i++){
      for(j=0;j<DIM;j++){
        ...
        f     [i+I*aa+bb][j] = HARTREE_BOHR2MD*QMgrad[i+qm->nrQMatoms][j];
        fshift[i+I*aa+bb][j] = HARTREE_BOHR2MD*QMgrad[i+qm->nrQMatoms][j];
        QMgrad[i][j]=0.0; QMgrad[i+qm->nrQMatoms][j]=0.0;
      }
    }
    for(i=0;i<mm->nrMMatoms;i++){
      for(j=0;j<DIM;j++){
        f     [i+qm->nrQMatoms+I*aa][j] = HARTREE_BOHR2MD*MMgrad[i][j];      
        fshift[i+qm->nrQMatoms+I*aa][j] = HARTREE_BOHR2MD*MMgrad[i][j];
        MMgrad[i][j]=0.0;
      }
    }
  }
