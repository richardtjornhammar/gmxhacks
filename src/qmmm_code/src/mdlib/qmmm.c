...
*/
  int
    i;
  // Keep unshifted coordinates for clustering
  // NaN is defined via math.h
  for(i=0;i<qm->nrQMatoms;i++){
    if( isnan(norm(x[qm->indexQM[i]])) ){
      fprintf(stderr,"WARNING :: YOUR SYSTEM IS BROKEN!\nRETRIEVED:: %d %lf %lf %lf\n",qm->indexQM[i],x[qm->indexQM[i]][XX],x[qm->indexQM[i]][YY],x[qm->indexQM[i]][ZZ] );
      fprintf(stderr,"           THIS HAPPENS WHEN MPI IS NOT A THREAD SAFE IMPLEMENTATION\n");
      gmx_fatal(FARGS,"ERROR :: 'NaN' COORDINATES WERE FOUND\n");
    }else{
		       ...
		       gmx_mtop_t *mtop, t_inputrec *ir)
{
  ...
  qm->QMfunctional   = ir->QMfunctional;    //ONLY USED IN MPQC INTERFACE
  qm->bQMdispersion  = ir->bQMdispersion;
  qm->fQMscreenK     = ir->fQMscreenK;
  qm->fQMscreenP     = ir->fQMscreenP;
  qm->fQMscreenPow   = ir->fQMscreenPow;
  qm->fQMscreenS     = ir->fQMscreenS;
  qm->fQMscreenF     = ir->fQMscreenF;
  qm->fMMgradMask    = ir->fMMgradMask;
  qm->fQMgradMask    = ir->fQMgradMask;
  qm->bQMexclusion   = ir->bQMexclusion;
  qm->fQMexclusion   = ir->fQMexclusion;

// qm->fQMscreenS-qm->fQMscreenF*exp(-1.0*qm->fQMscreenK*pow( R - qm->fQMscreenP , qm->fQMscreenPow) )

  fprintf(stderr,"QMPC SCREEN:: %9.3f / %9.3f / %9.3f / %9.3f / %9.3f:: Fq{1.0,0.5,0.3,0.2} = {%9.3f,%9.3f,%9.3f,%9.3f}\n", 
  qm->fQMscreenK, qm->fQMscreenP, qm->fQMscreenS, qm->fQMscreenF,qm->fQMscreenPow,
  qm->fQMscreenS-qm->fQMscreenF*exp(-1.0*qm->fQMscreenK*pow( 1.0 - qm->fQMscreenP , qm->fQMscreenPow) ),
  qm->fQMscreenS-qm->fQMscreenF*exp(-1.0*qm->fQMscreenK*pow( 0.5 - qm->fQMscreenP , qm->fQMscreenPow) ),
  qm->fQMscreenS-qm->fQMscreenF*exp(-1.0*qm->fQMscreenK*pow( 0.3 - qm->fQMscreenP , qm->fQMscreenPow) ),
  qm->fQMscreenS-qm->fQMscreenF*exp(-1.0*qm->fQMscreenK*pow( 0.2 - qm->fQMscreenP , qm->fQMscreenPow) ) ); 

  qm -> nQMclusterupdate = (qm -> nQMclusterupdate==-1)?(-1):(ir -> nQMclusterupdate);
  ...
  if(qm->QMengine==eMPQC || qm->QMengine==eGAUSSIAN){
    ...
    //snew(qm->fcharges,(nr+1)*nr);
    snew(qm->rQM,nr*(nr+1));
    if(qm->nQMclusterupdate != (-1)) {
    if(qm->bCLUSTER){	
		  ...
		  t_forcerec *fr)
{
  ...
  snew(qr->qm,jmax);
  if(qr->QMMMscheme!=eQMMMschemeoniom && (jmax>1) && (ir->bCLUSTER) ){
    qr->qm[0] = mk_QMrec();
    // This means that we are doing clustering with fixed clusters equal to the specified groups
    // Note that all atoms are placed in the clustering arrays of the first group
    fprintf(stderr,"CLUSTERING IS NOT UPDATED AND SET BY USERS %d QM GROUPS\n",jmax);
    qr->qm[0]->nQMclusterupdate=-1;  // We keep the qm parts separate throughout the simulation
    qr->qm[0]->nrQMc = jmax;         // We have user defined amount of clusters (no searching)
  }
  for(j=0;j<jmax;j++){
    /* new layer */
	...
	srenew(qm_arr,qm_max);
        ...
        if(qr->QMMMscheme!=eQMMMschemeoniom && (jmax>1) && (ir->bCLUSTER) ){
          srenew(qr->qm[0]->QMcluster,jmax*qm_max); // Nr of qm atoms times the number of clusters
          srenew(qr->qm[0]->QMcA,qm_max*qm_max);    // Used for MM clustering
          srenew(qr->qm[0]->QMcB,qm_max*qm_max);    // Used for MM clustering
          srenew(qr->qm[0]->QMcV,qm_max);           // Used for post distance matrix calculations           
        }
      }
	...
	qm_arr[undx] = i;
        if(qr->QMMMscheme!=eQMMMschemeoniom && (jmax>1) && (ir->bCLUSTER) ){
          qr->qm[0]->QMcluster[2*undx]   = j;
          qr->qm[0]->QMcluster[2*undx+1] = undx;      
        }
        if(groups->grpnr[egcQMskip]==NULL){
          useqm_arr[undx]=1;
        }else{
          useqm_arr[undx]= (ggrpnr(groups,egcQMskip ,i) != j)?1:0;
        }
      }
    }
    if(qr->QMMMscheme==eQMMMschemeoniom){
  ...
  //      fprintf(stderr,"GOT HERE -- 0\n");
    ...
    if(!(qr->QMMMscheme!=eQMMMschemeoniom && (jmax>1) && (ir->bCLUSTER)) ){ //it was already created
      qr->qm[0] = mk_QMrec();
    }
    ...
    for (k=0;k<qm_nr;k++){
      ...
      pchrg[k*2]=atom->q;
      pchrg[k*2+1]=atom->qB;
      atom->q  =  (atom->q) * ( ( fr->bQMlambda ? 1.0 : (0.0) ) ); // DURING TI THE USER MUST SET 
      atom->qB = (atom->qB) * ( ( fr->bQMlambda ? 1.0 : (0.0) ) ); // THE QM STATE CHARGES TO ZERO
    }
    
    for(k=0;k<qm_nr;k++){
      qr->qm[0]->pcharges[k]=pchrg[k*2];
      qr->qm[0]->useQMatom[k]=useqm_arr[k];
      if( (qr->qm[0]->useQMatom[k] > 1 || qr->qm[0]->useQMatom[k] < 0) || !( (qr->qm[0]->atomicnumberQM[k] == 0) && qr->qm[0]->useQMatom[k] == 0 ) ){ // can only switch off LA or dummy atoms
        qr->qm[0]->useQMatom[k]=1;
      }
    // add for debug:
    //  fprintf(stderr,"QMatom nr:: %d Use:: %d/%d Z:: %d\n",k+1,qr->qm[0]->useQMatom[k],useqm_arr[k],qr->qm[0]->atomicnumberQM[k]) ;
    ...
    for(i=0;i<qm_nr;i++){
      ...
      //NEEDED FOR DISPERION CORRECTION OF Q ATOMS
      qr->qm[0]->c6[i]  =  C6(fr->nbfp,mtop->ffparams.atnr,atom->type,atom->type)/((qr->qm[0]->bCLUSTER)?1.0:c6au); 
 ...
 //fprintf(stderr,"GOT HERE\n\n");
    ...
    if( (qr->qm[0]->QMengine==eMPQC || qr->qm[0]->QMengine==eGAUSSIAN) && (qr->qm[0]->bCLUSTER)) {
      ...
      snew(mm->fcharges,(mtop->natoms)*(qr->qm[0]->nrQMatoms));
		    ...
		    gmx_localtop_t *top)
{
  ...
  /*  needs to be called first, see pbc.h */
  ...
  if(qr->QMMMscheme==eQMMMschemenormal){ /* also implies 1 QM-layer */
    ...
    if(QMMMlist_sr.nri>0){     // added >0 so init::NULL implies no shift
 			       ...
 			       // THIS WILL BE INADEQUATE FOR CLUSTERING
      qm_i_particles[0].shift = XYZ2IS(0,0,0); 
      for(i=0;i<QMMMlist_sr.nri;i++){
	qm_i_particles[i].j     = QMMMlist_sr.iinr[i];
	if( !(i == 0) ){
	  qm_i_particles[i].shift = pbc_dx_aiuc(&pbc,x[QMMMlist_sr.iinr[0]],x[QMMMlist_sr.iinr[i]],dx);
	}
    ...
    for(i=0;i<mm->nrMMatoms;i++){   // QM SYSTEM MIGHT BE INTERACTING WITH A SYSTEM UNDERGOING TI 
      if(fr->efep == efepNO){
        mm->MMcharges[i]=(md->chargeA[mm->indexMM[i]]);
      }else{                        // LAMBDA0 IS STATEA 1 IS B
        mm->MMcharges[i]=( (md->chargeA[mm->indexMM[i]])*(1.0-(md->lambda))+(md->lambda)*(md->chargeB[mm->indexMM[i]]) ); 
      }
    } 
    if(qm->bTS||qm->bOPT){
    ...
    if( (qm->bCLUSTER && (qm->QMengine==eMPQC || qm->QMengine==eGAUSSIAN)) ){
      replicate_QMMM_coord(x,fr,qr->qm[0],qr->mm); //need to check
    }else{
      update_QMMM_coord(x,fr,qr->qm[0],qr->mm);
    }

    // BEGIN NEW CLUSTERING
    qm->dI=0;
    if(qm->bCLUSTER){
      srenew(qm->QMcIS,(qm->nrQMatoms+1)*(qm->nrQMatoms));
      srenew(mm->MMcIS,(qm->nrQMatoms+mm->nrMMatoms+1)*(qm->nrQMatoms));
      clustering(qm,ppbc);  
      clusterMM(mm,qm,ppbc);
    }
    else{
      qm->nrQMc=1;
      srenew(qm->QMcIS,(qm->nrQMatoms+1));
      srenew(mm->MMcIS,(qm->nrQMatoms+mm->nrMMatoms+1)); 
      one_cluster(mm,qm);  
      clusterMM(mm,qm,ppbc); 
    }
    // END NEW CLUSTERING
		    ...
		    t_mdatoms *md)
{
   ...
   */
 // fprintf(stderr,"inside calculation routine\n");
  if(qr->QMMMscheme==eQMMMschemenormal || qr->nrQMlayers==1){
    ...
    nrT=nrMM+nrQM*2;
    if(qm->bCLUSTER){
      snew(forces,nrT*nrQM); 
      snew(fshift,nrT*nrQM);
    }else{
      snew(forces,nrT+1);
      snew(fshift,nrT+1);
    }
   // fprintf(stderr,"calling mpqc\n");
...
/*    if(!(qm->bCLUSTER)){
	  ...
	  fr->fshift[(qm->shiftQM[i])][j] += (fshift[i][j])*(fr->bQMlambda?fr->qmlambda:1.0); 
        }
      }
      for(i=0;i<mm->nrMMatoms;i++){
        for(j=0;j<DIM;j++){
            f[mm->indexMM[i]][j]            -= forces[qm->nrQMatoms+i][j]*(fr->bQMlambda?fr->qmlambda:1.0);
            fr->fshift[(mm->shiftMM[i])][j] += fshift[qm->nrQMatoms+i][j]*(fr->bQMlambda?fr->qmlambda:1.0); 
        }
      }
    }else{
*/
      for(I=0;I<qm->nrQMc;I++){
        ...
        for(i=0;i<qm->nrQMatoms;i++){ //QM and QA
          for(j=0;j<DIM;j++){
            f[qm->indexQM[i]][j]                 -= (forces[I*nrT+i][j])*(fr->bQMlambda?fr->qmlambda:1.0);
	    fr->fshift[(qm->QMcIS[I*nrQM+i])][j] += (fshift[I*nrT+i][j])*(fr->bQMlambda?fr->qmlambda:1.0); 
            f[qm->indexQM[i]][j]                             -= (forces[I*nrT+i+nrQM+nrMM][j])*(fr->bQMlambda?fr->qmlambda:1.0);
            fr->fshift[(mm->MMcIS[i+nrMM+I*(nrQM+nrMM)])][j] += (fshift[I*nrT+i+nrQM+nrMM][j])*(fr->bQMlambda?fr->qmlambda:1.0);  
        ...
        for(i=0;i<mm->nrMMatoms;i++){
          for(j=0;j<DIM;j++){
            f[mm->indexMM[i]][j]                        -= forces[I*nrT+nrQM+i][j]*(fr->bQMlambda?fr->qmlambda:1.0);
            fr->fshift[(mm->MMcIS[I*(nrQM+nrMM)+i])][j] += fshift[I*nrT+nrQM+i][j]*(fr->bQMlambda?fr->qmlambda:1.0); 
          }
        }
