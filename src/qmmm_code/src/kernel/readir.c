/*
 * $Id: readir.c,v 1.252.2.10 2009/02/16 10:10:08 hess Exp $
 * 
...
static char tcgrps[STRLEN],tau_t[STRLEN],ref_t[STRLEN],
  ...
  wall_atomtype[STRLEN],wall_density[STRLEN],deform[STRLEN],QMMM[STRLEN],skipQM[STRLEN];
	    ...
	    t_inputrec *ir,t_gromppopts *opts,int *nerror)
{
  ...
  CCTYPE ("OPTION FOR MAKING QM CLUSTERING WITH MPQC");
  EETYPE("QMclustering", ir->bCLUSTER, yesno_names, nerror, TRUE);
  CTYPE ("Groups treated Quantum Mechanically");
  STYPE ("QMMM-grps",  QMMM,          NULL);
  CTYPE ("QM method");
  STYPE("QMmethod",     QMmethod, NULL);
  CTYPE("QM engine");
  EETYPE("QMengine",    ir->QMengine, eQMengine_names, nerror, TRUE);
  CTYPE("QM functional");
  EETYPE("QMfunctional",    ir->QMfunctional, eQMfunctional_names, nerror, TRUE);
  CTYPE("QM pointcharges");
  EETYPE("QMincludeQ",    ir->QMincludeQ, eQMincludeQ_names, nerror, TRUE); 
  CTYPE ("QMMM scheme");
  EETYPE("QMMMscheme",  ir->QMMMscheme,    eQMMMscheme_names, nerror, TRUE);
  CTYPE ("QM basisset");
  CTYPE("QMlambda (1.0 is ON, 0.0 is OFF) ");
  EETYPE("bQMlambda", ir->bQMlambda, yesno_names,nerror, TRUE);
  EETYPE("bQMrestart", ir->bQMrestart, yesno_names,nerror, TRUE);
  EETYPE("bQMsavestate", ir->bQMsavestate, yesno_names,nerror, TRUE);
  EETYPE("print_npa", ir->bPrint_npa, yesno_names, nerror, TRUE);
  EETYPE("bQMcheckpoint", ir->bQMcheckpoint, yesno_names,nerror, TRUE);
  CTYPE ("Guess Wavefunction with (big) 6-31G or (small) STO-3G guess");
  EETYPE("GuessWF", ir->bGuessWF, eGUESSWF_names, nerror, TRUE);
  RTYPE("QMlambda", ir->qmlambda, 1.0);
  STYPE("QMbasis",      QMbasis, NULL);
  RTYPE("level_shift", ir->QM_level_shift, 0.0);
  CTYPE ("QM charge");
  ...
  ITYPE ("nQMcenter", ir->nQMcenter, 0);
  ITYPE ("nQMclusterupdate", ir->nQMclusterupdate, 0);
  RTYPE ("fQMcutoff", ir->fQMcutoff, 0.3);
  RTYPE ("fQMelrc",ir->fQMelrc,1.0);
  RTYPE ("fQMscreenK", ir->fQMscreenK, 1000.0);
  RTYPE ("fQMscreenP", ir->fQMscreenP, 0.0);
  RTYPE ("fQMscreenPow", ir->fQMscreenPow, 1.0);
  RTYPE ("fQMscreenS", ir->fQMscreenS, 1.0);
  RTYPE ("fQMscreenF", ir->fQMscreenF, 1.0);
  RTYPE ("fMMgradMask", ir->fMMgradMask, 1.0);
  RTYPE ("fQMgradMask", ir->fQMgradMask, 1.0);
  RTYPE ("fQMexclusion", ir->fQMexclusion, 0.0 );
  EETYPE("bQMexclusion", ir->bQMexclusion, yesno_names, nerror, TRUE); 
  EETYPE("bQMdispersion", ir->bQMdispersion, yesno_names,nerror, TRUE);
  EETYPE("bQMadhoc", ir->bQMadh, yesno_names,nerror, TRUE);
  STYPE ("skipQM-grp",  skipQM,          NULL);
...
#undef CTYPE
  ...
  if(ir->bQMMM){
    if(!(ir->init_lambda<0.0 || ir->init_lambda > 1.0)){
      ir->qmlambda=ir->init_lambda;
    }
    else{
     gmx_fatal(FARGS,"QM_lambda=%10.6f is not a supported value (0.0 -> 1.0)", ir->init_lambda);
    }
    if(ir->bQMlambda && (ir->qmlambda<0.0 || ir->qmlambda > 1.0)){
       gmx_fatal(FARGS,"QM_lambda=%10.6f is not a supported value (0.0 -> 1.0)", ir->qmlambda);
    }
    if(ir->bQMadh && !(ir->bQMlambda) ){
       ir->bQMadh=0;
    }
    if(ir->bQMlambda){
      fprintf(stdout,"Have (QM,MM) = (%10.6f \%,%10.6f \%)\n",(ir->qmlambda)*100,(1.0-ir->init_lambda)*100);
    }
    fprintf(stdout,"Have center id = %d\n",(ir->nQMcenter));
    if(ir->bCLUSTER){
      fprintf(stdout,"Doing clustering with r_c = %10.6f\n",(ir->fQMcutoff));
      if(ir->nQMclusterupdate<0){
        fprintf(stdout,"WARNING NEGATIVE UPDATE NOT PERMITTED SO SET TO ZERO\n");
        ir->nQMclusterupdate=0;
      }
    }
  }
	      ...
	      t_inputrec *ir,rvec *v)
{
  ...
  /* begin skipQM input */
  nr = str_nelem(skipQM,MAXPTR,ptr1);
  if(nr>1){
    gmx_fatal(FARGS,"Should only have one skip group");
  }else{
    fprintf(stderr," Added %d qmskips(s)\n",nr); 
  }
  do_numbering(natoms,groups,nr,ptr1,grps,gnames,egcQMskip,
               restnm,egrptpALL_GENREST,bVerbose);
  /* end skipQM input */
