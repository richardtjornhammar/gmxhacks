/*
 * $Id: tpxio.c,v 1.178.2.1 2008/10/08 01:33:43 lindahl Exp $
 * 
			...
			real *fudgeQQ)
{
  ...
  if (file_version >= 1) {  
    ...
    if(file_version>=39){
      ...
      do_int(ir->bQMlambda);
      do_int(ir->bQMadh);
      do_int(ir->QMfunctional);
      do_int(ir->QMMMscheme);
      do_real(ir->fQMscreenK);
      do_real(ir->fQMscreenP);
      do_real(ir->fQMscreenS);
      do_real(ir->fQMscreenF);
      do_real(ir->fQMscreenPow);
      do_real(ir->fQMexclusion);
      do_int (ir->bQMexclusion);
      do_real(ir->fQMgradMask);
      do_real(ir->fMMgradMask);
      do_int(ir->QMengine);
      do_int(ir->QMincludeQ);
      do_real(ir->scalefactor);
      do_int(ir->opts.ngQM);
      do_real(ir->qmlambda);
      do_int(ir->bGuessWF);
      do_int(ir->bQMrestart);
      do_int(ir->bQMcheckpoint);
      do_int(ir->bQMsavestate);
      do_int(ir->bPrint_npa);
      do_int(ir->bQMdispersion);
      do_real(ir->QM_level_shift);
      do_int(ir->nQMcenter);
      do_int(ir->nQMclusterupdate);
      do_real(ir->fQMcutoff);
      do_real(ir->fQMelrc);
      if (bRead) {
