		 ...
		 int *global_atom_index)
{
  const real one=1.0;
  const real two=2.0;
  real  dr,dr2,temp,omtemp,cbomtemp,fbond,vbond,fij,b0,be,cb,vtot,fact=1.0;
  rvec  dx;
  ...
  for(i=0; (i<nbonds); ) {
    ...
    if(md->qmmm_integration){
      fact=( ( md->bQMadh && md->bQMatom[ai] && md->bQMatom[aj])?(1.0-md->lambdaQM):1.0 );
    }else{
      fact=1.0;
    }
		 ...
		 int *global_atom_index)
{
  ...
  const real two   = 2.0;
  real  kb,b0,kcub;
  real  dr,dr2,dist,kdist,kdist2,fbond,vbond,fij,vtot,fact=1.0;
  rvec  dx;
  int   i,m,ki,type,ai,aj;
  ivec  dt;
		...
		int *global_atom_index)
{
  ...
  const real one=1.0;
  real  bm,kb;
  real  dr,dr2,bm2,omdr2obm2,fbond,vbond,fij,vtot,fact=1.0;
  rvec  dx;
  int   i,m,ki,type,ai,aj;
  ivec  dt;
	   ...
	   int *global_atom_index)
{
  ...
  real dr,dr2,fbond,vbond,fij,vtot,fact=1.0;
  rvec dx;
  ivec dt;
...
{
  int  i,m,ki,ai,aj,type;
  real dr,dr2,fbond,vbond,fij,vtot,ksh,fact=1.0;
  rvec dx;
  ivec dt;
	    ...
	    int *global_atom_index)
{
  ...
  rvec r_ij,r_kj;
  real cos_theta,cos_theta2,theta,dVdt,va,vtot,fact=1.0;
  ivec jt,dt_ij,dt_kj;
  ...
  for(i=0; (i<nbonds); ) {
    ...
    if(md->qmmm_integration){
      fact=( ( md->bQMadh && md->bQMatom[ai] && md->bQMatom[aj] && md->bQMatom[ak]) ? (1.0-md->lambdaQM):1.0 );
    }else{
      fact=1.0;
    }
    ...
    *dvdlambda *= fact;
	...
	fprintf(debug,"ANGLES: theta = %10g  vth = %10g  dV/dtheta = %10g\n",
      ...
      for (m=0; (m<DIM); m++) {			/*  39		*/
	f_i[m]=-(cik*r_kj[m]-cii*r_ij[m])*fact;
	f_k[m]=-(cik*r_ij[m]-ckk*r_kj[m])*fact;
	f_j[m]=(-f_i[m]-f_k[m])*fact;
	f[ai][m]+=f_i[m];
	f[aj][m]+=f_j[m];
	f[ak][m]+=f_k[m];
      }
		  ...
		  int *global_atom_index)
{
  ...
  real dVdt,va,vtot,kth,th0,kUB,r13,dr,dr2,vbond,fbond,fik,fact=1.0;
		    ...
		    int *global_atom_index)
{
  ...
  rvec r_ij,r_kj;
  real cos_theta,cos_theta2,theta,dt,dVdt,va,dtp,c,vtot,fact=1.0;
  ivec jt,dt_ij,dt_kj;
...
void do_dih_fup_s(int i,int j,int k,int l,real ddphi,
		rvec r_ij,rvec r_kj,rvec r_kl,
		rvec m,rvec n,rvec f[],rvec fshift[],
		const t_pbc *pbc,const t_graph *g,
		const rvec x[],int t1,int t2,int t3,real s)
{
  /* 143 FLOPS */
  rvec f_i,f_j,f_k,f_l;
  rvec uvec,vvec,svec,dx_jl;
	   ...
	   int *global_atom_index)
{
  ...
  real phi,cos_phi,sign,ddphi,vpd,vtot,fact=1.0;
  ...
  for(i=0; (i<nbonds); ) {
    ...
    if(md->qmmm_integration){
      fact=( ( md->bQMadh && md->bQMatom[ai] && md->bQMatom[aj] && md->bQMatom[ak] && md->bQMatom[al]) ? (1.0-md->lambdaQM):1.0 );
    }else{
      fact=1.0;
    }
    ...
    *dvdlambda *=fact; 		       
    ...
    if(md->qmmm_integration){
      do_dih_fup_s(ai,aj,ak,al,ddphi,r_ij,r_kj,r_kl,m,n,
	       f,fshift,pbc,g,x,t1,t2,t3,fact);
    }else{
	   ...
	   int *global_atom_index)
{
  int  i,type,ai,aj,ak,al;
  int  t1,t2,t3;
  real phi,phi0,dphi0,cos_phi,ddphi,sign,vtot,fact=1.0;
  rvec r_ij,r_kj,r_kl,m,n;
  ...
  for(i=0; (i<nbonds); ) {
    ...
    vtot += 0.5*kk*dp2*fact;
    ddphi = -kk*dp;
    
    dvdl += 0.5*(kB - kA)*dp2 - kk*dphi0*dp;

    if(md->qmmm_integration){
       do_dih_fup_s(ai,aj,ak,al,(real)(-ddphi),r_ij,r_kj,r_kl,m,n,
	       f,fshift,pbc,g,x,t1,t2,t3,fact);     
    }else{
...
{
  int  i,m,ki,ai,aj,type;
  real dr2,fbond,vbond,fij,vtot,fact=1.0;
  rvec dx;
  ivec dt;
	       ...
	       int *global_atom_index)
{
  ...
  rvec r_ij,r_kj;
  real cos_theta,dVdt,va,vtot,fact=1.0;
...
#endif
    for (m=0; (m<DIM); m++) {			/*  42	*/
      f_i[m]=dVdt*fact*(r_kj[m]*rijrkj_1 - r_ij[m]*rij_2*cos_theta);
      f_k[m]=dVdt*fact*(r_ij[m]*rijrkj_1 - r_kj[m]*rkj_2*cos_theta);
      f_j[m]=(-f_i[m]-f_k[m])*fact;
      f[ai][m]+=f_i[m];
      f[aj][m]+=f_j[m];
      f[ak][m]+=f_k[m];
		     ...
		     int *global_atom_index)
{
  ...
  int  i,ai,aj,ak,type,m,t1,t2;
  rvec r_ij,r_kj;
  real vtot,vrr,s1,s2,r1,r2,r1e,r2e,krr,fact=1.0;
  rvec f_i,f_j,f_k;
  ivec jt,dt_ij,dt_kj;
  ...
  for(i=0; (i<nbonds); ) {
    ...
    /* Forces */
    svmul(-krr*s2*fact/r1,r_ij,f_i);
    svmul(-krr*s1*fact/r2,r_kj,f_k);
		      ...
		      int *global_atom_index)
{
  ...
  rvec r_ij,r_kj,r_ik;
  real vtot,vrt,s1,s2,s3,r1,r2,r3,r1e,r2e,r3e,krt,k1,k2,k3,fact=1.0;
  rvec f_i,f_j,f_k;
  ivec jt,dt_ij,dt_kj;
  ...
  for(i=0; (i<nbonds); ) {
    ...
    *dvdlambda *= fact;
	...
	fprintf(debug,"ANGLES: theta = %10g  vth = %10g  dV/dtheta = %10g\n",
      ...
      cik=st*invsqrt(nrkj2*nrij2)*fact;		/*  12		*/ 
      cii=sth/nrij2*fact;				/*  10		*/
      ckk=sth/nrkj2*fact;				/*  10		*/
