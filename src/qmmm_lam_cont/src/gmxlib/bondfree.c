		 ...
		 int *global_atom_index)
{
  ...
  for(i=0; (i<nbonds); ) {
    ...
    if(md->qmmm_integration){
      fact=( (md->bQMatom[ai] || md->bQMatom[aj])?(1.0-md->lambdaQM):1.0 );
    }else{
      fact=1.0;
    }
	    ...
	    int *global_atom_index)
{
  ...
  for(i=0; (i<nbonds); ) {
    ...
    if(md->qmmm_integration){
      fact=( (md->bQMatom[ai] || md->bQMatom[aj] || md->bQMatom[ak]) ? (1.0-md->lambdaQM):1.0 );
    }else{
      fact=1.0;
    }
	   ...
	   int *global_atom_index)
{
  ...
  for(i=0; (i<nbonds); ) {
    ...
    if(md->qmmm_integration){
      fact=( (md->bQMatom[ai] || md->bQMatom[aj] || md->bQMatom[ak] || md->bQMatom[al]) ? (1.0-md->lambdaQM):1.0 );
    }else{
      fact=1.0;
    }
