...
void write_gaussian_input(int step ,t_forcerec *fr, t_QMrec *qm, t_MMrec *mm)
{
  ...
  for(cl=0;cl<NRC;cl++){
    ...
    //fprintf(stderr,"(% 9.6f % 9.6f",tchrg[cl], chrg);
    chrg=tchrg[cl];
    //fprintf(stderr," | % 4d, % 4d )\n",(int)roundChrg( chrg ) , multip[cl] );
  ...
  for(cl=0;cl<NRC;cl++){
    ...
    /* MM point charge data */
    if(QMMMrec->QMMMscheme!=eQMMMschemeoniom && mm->nrMMatoms && (qm->includeQ>eIncludeNONE)){
      if(cl==0){
...
real read_gaussian_output(rvec QMgrad[],rvec MMgrad[],int step,t_QMrec *qm, t_MMrec *mm, int I)
{
  ...
  if(qm->QMmethod>=eQMmethodRHF && qm->includeQ>eIncludeNONE && nrMM>0){  
    l=0;
    for(i=0;i<nrMM;i++){
...
void do_gaussian(int step, char *exe, int I)
{
  ...
  if ( system(buf) != 0 ){
	...
	while(!feof(com)){
	  ...
	  if(line[0]=='#'){
	    sscanf(line,"%s %s",st1,st2);
	    fprintf(newcom,"%s %s Guess=Mix SCF=NoDIIS\n",st1,st2);
	  }
	  else
	    fprintf(newcom,"%s",line);
	}
	...
	while(!feof(com)){
	  ...
	  if(line[0]=='#'){
	    sscanf(line,"%s %s",st1,st2);
	    fprintf(newcom,"%s %s Guess=Mix SCF=NoDIIS Charge\n",st1,st2);
	  }
	  else
	    fprintf(newcom,"%s",line);
	}
	...
	while(!feof(com)){
	  ...
	  if(line[0]=='#'){
	    sscanf(line,"%s %s",st1,st2);
	    fprintf(newcom,"%s %s Guess=Mix SCF=QC Charge\n",st1,st2);
	  }
	  else
	    fprintf(newcom,"%s",line);
	}
