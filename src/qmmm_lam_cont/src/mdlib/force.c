                   ...
                   real       print_force)
{
    ...
    fr->bQMMM      = ir->bQMMM;   
    fr->qr         = mk_QMMMrec();
    fr->bQMlambda  = ir->bQMlambda;
    fr->qmlambda   = ir->qmlambda;
...
/*
    if(fr->bQMMM && fr->bQMlambda){
    // dummy force vectors for qm->mm lambda integration 
        snew(fd_qm,(md->nr));
        snew(fd_nb,(md->nr));
        snew(fd_bd,(md->nr));
        snew(bfQM, (md->nr));
        ...
        for(k=0;k<fr->qr->nrQMlayers;k++){
         for(j=0;j<fr->qr->qm[k]->nrQMatoms;j++){
             bfQM[fr->qr->qm[k]->indexQM[j]]=1;
         }
       }
    }
*/
    ...
    if(fr->bQMMM)
    {
        enerd->term[F_EQM] = calculate_QMMM(cr,x,f,fr,md);//((fr->bQMlambda)?fd_qm:f),fr,md);
    }
...
#define TAKETIME FALSE
    ...
    where();
    do_nonbonded(cr,fr,x,f,md,//((fr->bQMlambda)?fd_nb:f),md,
                 fr->bBHAM ?
    ...
    if (flags & GMX_FORCE_BONDED)
    {
        ...
        calc_bonds(fplog,cr->ms,
                   idef,x,hist,f,fr,&pbc,graph,enerd,nrnb,lambda,md,fcd,//((fr->bQMlambda)?fd_bd:f),fr,&pbc,graph,enerd,nrnb,lambda,md,fcd,
    ...
    /* done with this part so assign */
    // BEFORE THE SCALING WAS DONE HERE NOT INSIDE KERNELS :: PERHAPS WANT TO SCALE NB
/*
    if(fr->bQMMM && fr->bQMlambda){
        for(j=0;j<md->nr;j++){
           for(k=XX;k<=ZZ;k++)
             f[j][k]+=fd_qm[j][k]+fd_nb[j][k]+fd_bd[j][k];
        }
    }
*/
    where();
