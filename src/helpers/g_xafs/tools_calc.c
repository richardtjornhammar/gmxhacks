...
void sortLL(LINK *AR, int nrEntries) {
  int sa,sb;
  float ft,ft2;
  ...
  for(sa = 1; sa < nrEntries; ++sa)
    for(sb = nrEntries-1; sb >= sa; --sb) {
      // compare adjacent elements 
      if( (AR[sb - 1] -> data.r) > (AR[sb]->data.r) ) {
	// exchange elements 
	swapAtoms(AR[sb-1],AR[sb]);
      }
    }
}
...
float rmPBC(LINK at,LINK cat, float CELL[6])
{
  int i,j;
  float sign;
  float e[3],E[3],B[3],ff,maxL=0.0;
  float R[3],rlen=0.0,rtem=0.0;

  for(j=0;j<3;j++) //CENTERS COORDINATES AROUND CENTRAL ATOM
    {
      R[j]=at->data.pos[j]-cat->data.pos[j];
      R[j]-=CELL[j]*anint(R[j]/CELL[j]);  
      at->data.pos[j]=R[j];
      rtem+=pow(R[j],2.0);
    }
  rlen=sqrt(rtem);
...
void scale(LINK at,LINK cat, float rsc)
{
  int i,j;
  ...
  rlen=sqrt(rtem);
  for(j=0;j<3;j++){
    at->data.pos[j]+=(rsc-1.0)*R[j]/rlen;
    fprintf(stdout,"%f ",at->data.pos[j]);
  }
  fprintf(stdout,"| %f %f ",rlen,rsc);
}

float rmPBCsol(LINK at,LINK cat, float CELL[6])
{
  int i,j;
  float e[3],E[3],B[3],ff;
  float R[3],rlen=0.0,rtem=0.0;
  ...
  for(j=0;j<3;j++) 
    {
      R[j]=H1->data.pos[j]-at->data.pos[j];
      if(R[j]>CELL[j]*0.5)
	R[j]-=CELL[j];
      else
	if(abs(R[j])>CELL[j]*0.5)
  ...
  for(j=0;j<3;j++)
    {
      R[j]=H2->data.pos[j]-at->data.pos[j];
      if(R[j]>CELL[j]*0.5)
	R[j]-=CELL[j];
      else
	if(abs(R[j])>CELL[j]*0.5)
