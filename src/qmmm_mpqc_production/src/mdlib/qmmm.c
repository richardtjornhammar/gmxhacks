		       ...
		       gmx_mtop_t *mtop, t_inputrec *ir)
{
  ...
  if(qm->QMengine==eMPQC){
    qm->includeQ=ir->QMincludeQ;
    //    fprintf(stderr,"IS INSIDE MPQC DEPENDENT CODE SNIPPET::WILL ALLOCATE>>\n");
    snew(qm->pcharges,nr);
    //    fprintf(stderr,"PCHARGES         [ DONE ]\n");
    snew(qm->rQM,nr*nr);
    //    fprintf(stderr,"POSITION VECTOR  [ DONE ]\n");
    if(qm->bCLUSTER){
      //      fprintf(stderr,"Will do clustering of QM atoms\n");	
      snew(qm->QMcluster,nr*2*nr);/* Nr of qm atoms times the maximum number of clusters */
      //      fprintf(stderr,"QMCLUSTER VECTOR [ DONE ]\n");
      snew(qm->QMcluspos,nr*nr)  ;/* Nr of qm atoms times the maximum number of clusters */
      //      fprintf(stderr,"QMCLUSPOS VECTOR [ DONE ]\n");
      snew(qm->QMcA,nr*nr);
      //      fprintf(stderr,"REAL DISTANCE M. [ DONE ]\n");
      snew(qm->QMcB,nr*nr);
      //      fprintf(stderr,"BOOL DISTANCE M. [ DONE ]\n");
      snew(qm->QMcV,nr);
      //      fprintf(stderr,"VISITED VECTOR   [ DONE ]\n");
    }
    else {
      snew(qm->QMcluster,nr*2);
    }
  }
} /* init_QMrec */  
		  ...
		  t_forcerec *fr)
{
  ...
  if(qr->QMMMscheme!=eQMMMschemeoniom){
     ...
     */
    snew(pchrg,(qm_nr+1)*2);//=malloc(sizeof(real)*(qm_nr+1)*2);
    //    fprintf(stderr,"GOT HERE\n");
    for (k=0;k<qm_nr;k++){
      //SEGFAULTS IF NO MM :: qm_arr
      //PROBLEM NR2:: SHOULD NOT HAVE QM ATOMS IN ITP FILES::YES
      //fprintf(stderr,"INDEX ::  %d",k);
      gmx_mtop_atomnr_to_atom(mtop,qm_arr[k],&atom);
      pchrg[k*2]=atom->q;
      pchrg[k*2+1]=atom->qB;
	atom->q  = 0.0;
    ...
    //    fprintf(stderr,"GOT HERE\n\n");
    if((qr->qm[0]->QMengine==eMPQC) & (qr->qm[0]->bCLUSTER)) {
      //      fprintf(stderr,"WILL ALLOCATE FOR MM >>\n");
      snew(mm->MMcluster,(mm->nrMMatoms+qr->qm[0]->nrQMatoms)*(qr->qm[0]->nrQMatoms)); 
      //      fprintf(stderr,"MMCLUSTER VECTOR [ DONE ]\n");
      /* Nr of atoms times the maximum number of clusters    */
      for(i=XX;i<=ZZ;i++){
	for(j=XX;j<=ZZ;j++){
		    ...
		    gmx_localtop_t *top)
{
  ...
  if(qr->QMMMscheme==eQMMMschemenormal){ /* also implies 1 QM-layer */
    ...
    //NOTE: VERY UGLY SINCE CODE EASILY SEGFAULTS HERE
    //      CODE IS A PATCH::UNDERLYING PROBLEM STILL OCCURS IN CODE FOR ZERO MM QMMM RUNS  
    if(QMMMlist_sr.nri>0){
  ...
  if(qr->qm[0]->QMengine==eMPQC){
    snew(mm->rMM,(mm->nrMMatoms)*(qm->nrQMatoms));
    if(qr->qm[0]->bCLUSTER) {
      for(i=XX;i<=ZZ;i++){
	for(j=XX;j<=ZZ;j++){
	  qr->qm[0]->box[i][j]=box[i][j];
